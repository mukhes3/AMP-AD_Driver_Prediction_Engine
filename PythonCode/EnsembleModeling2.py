...
FileOut = 'EC2_Consensus_Layered_GW.csv'
...
X1 = pd.read_csv('./amp_ad_de_feature_set_mod.csv')
...
#d = {'0a':Y0a, '0b':Y0b, '1a':Y1a, '1b':Y1b, '2a':Y2a,
#    '2b':Y2b, '3a':Y3a, '3b':Y3b}

d = {'1a':Y1a, '1b':Y1b, '3a':Y3a, '3b':Y3b}

X_temp = pd.DataFrame(data = d)
poly = PolynomialFeatures(degree = 2)
X = pd.DataFrame(poly.fit_transform(X_temp))
X2 = X.abs()
X = X.values / X2.values.max(axis=0)
D = TrainingCrossVal(X,Y,'l2')
